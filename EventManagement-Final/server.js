...
/*
app.get('/userlogin', function (req, res) {
	db.query("SELECT * FROM booking JOIN users ON booking.users_id = users.users_id WHERE name = user_name;", function (err, result) {
		if (err) throw err;
		console.log(result);
		res.render('userlogin', { title: 'xyz', bookingData: result});
		});
	});
*/
...
app.post('/getbookings', function (req, res) {
    ...
    var sql = 'UPDATE eventbookings SET email = ? WHERE email = ?';

    db.query(sql, [new_value, email], function(err, result) {
        if (err) {
            console.error(err);
